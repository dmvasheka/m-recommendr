...
type InfiniteSearchPage<T> = { items: T[]; hasMore: boolean; page: number }
type InfiniteCursorPage<T> = { items: T[]; hasMore: boolean; nextCursor: string | null }
...
export function useInfiniteSearchMovies(params: SearchMoviesParams & { pageSize?: number }) {
    ...
    return useInfiniteQuery<InfiniteSearchPage<Movie>>({
...
export function useInfinitePopularMovies(pageSize = 20, language?: string) {
    return useInfiniteQuery<InfiniteCursorPage<Movie>>({
...
export function useInfiniteSearchTvShows(params: SearchTvShowsParams & { pageSize?: number }) {
    ...
    return useInfiniteQuery<InfiniteSearchPage<TvShow>>({
...
export function useInfiniteTvShows(pageSize = 20, language?: string) {
    return useInfiniteQuery<InfiniteCursorPage<TvShow>>({
